...
class Cube3DWidget(BoxLayout):
    ...
    def __init__(self, logic, get_current_number_fn, get_upcoming_fn, initial_filled=None, **kwargs):
        ...
        self.lbl_status_message = Label(
            size_hint_x=None, width=sp(300), font_size=sp(16), text="", halign="center", valign="middle"
        ...
        self.full_canvas.bind(
            on_touch_down=self._on_full_touch_down,
        ...
        ctrl.add_widget(btn_x)
        ctrl.add_widget(btn_z)
    ...
    def _on_number_skip_notification(self, skipped_number):
        ...
        popup = Popup(title="Number Skipped", content=popup_content, size_hint=(0.5, 0.3))
        popup.open()
    ...
    def _on_slice_touch_down(self, widget, touch):
        ...
        if widget is cvs and cvs.collide_point(*touch.pos):
            ...
            self.logic.field.logger.debug(
                f"DEBUG: cvs.x={cvs.x}, cvs.y={cvs.y}, cvs.width={cvs.width}, cvs.height={cvs.height}"
            )
            ...
            if 0 <= col < side and 0 <= row < side:
                ...
                if tuple(self.selected_index) == (i, j, k):
                    ...
                    if not success:
                        ...
                        Popup(
                            title="Invalid Input", content=Label(text=self.logic.get_last_error()), size_hint=(0.6, 0.3)
                        ).open()
                    else:
    ...
    def redraw(self, *args):
        ...
        cvs.canvas.clear()
        cvs.canvas.before.clear()
        with cvs.canvas.before:
            Color(1, 1, 1, 1)
            Rectangle(pos=cvs.pos, size=cvs.size)
        with cvs.canvas:
            # （既存の3D描画ロジックをそのまま流用）
            ...
            for i, x in enumerate(coords):
                for j, y in enumerate(coords):
                    for k, z in enumerate(coords):
                        ...
                        highlight = (self.slice_axis == "x" and i == self.slice_index) or (
                            self.slice_axis == "z" and k == self.slice_index
                        )
                        is_initial = initial_cell is not None
                        self._draw_box(
                            cvs, cx, cy, cz, num, highlight=highlight, error=False, layer=j, is_initial=is_initial
                        )
        # --- スライスビュー 2Dフラット (右) ---
        ...
        with cvs.canvas:
            ...
            for row in range(LENGTH_OF_SIDE):
                for col in range(LENGTH_OF_SIDE):
                    ...
                    Color(*bg, 1)
                    RoundedRectangle(pos=(x0, y0), size=(rect_w, rect_h), radius=[(8, 8), (8, 8), (8, 8), (8, 8)])
                    # 枠線＋選択セルハイライト
    ...
    def _draw_box(self, cvs, cx, cy, cz, num, highlight, error, layer, is_initial=False):
        half = self.box_size
        pts = [(cx + dx * half, cy + dy * half, cz + dz * half) for dx in (-1, 1) for dy in (-1, 1) for dz in (-1, 1)]
        ...
        if highlight:
            ...
            w = 0.8
        for a, b in edges:
            x1, y1 = self._project_to(cvs, *pts[a])
            x2, y2 = self._project_to(cvs, *pts[b])
            Line(points=[x1, y1, x2, y2], width=w)
        if num:
            sx, sy = self._project_to(cvs, cx, cy, cz)
            lbl = CoreLabel(text=str(num), font_size=18)
            lbl.refresh()
            Color(0, 0, 0, 1)
            Rectangle(
                texture=lbl.texture,
