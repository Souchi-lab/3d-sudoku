...
class TestInvalidMoves(unittest.TestCase):
    ...
    def test_place_on_initial_filled_cell(self):
        ...
        success = self.logic.attempt_input(
            initial_pos_data[0], initial_pos_data[1], initial_pos_data[2], initial_val + 1
        )
        self.assertFalse(success)
        self.assertEqual(self.logic.get_last_error(), "Invalid input: Already filled.")
        self.assertEqual(
            self.logic.get_number(initial_pos_data[0], initial_pos_data[1], initial_pos_data[2]), initial_val
        )  # Should remain the original number
