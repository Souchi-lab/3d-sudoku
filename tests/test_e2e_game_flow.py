...
class TestE2EGameFlow(unittest.TestCase):
    def setUp(self):
        ...
        self.patch_game_state = patch(
            "kivy_cube_app.services.game_controller.GameState", return_value=self.mock_game_state
        )
        self.patch_field_adapter = patch(
            "kivy_cube_app.services.game_controller.FieldAdapter", return_value=self.mock_field_adapter
        )
        self.patch_cube_logic = patch(
            "kivy_cube_app.services.game_controller.CubeLogic", return_value=self.mock_cube_logic
        )
        self.patch_cpu_player = patch(
            "kivy_cube_app.services.game_controller.CpuPlayer", return_value=self.mock_cpu_player
        )
        ...
        self.mock_game_state.get_score = MagicMock(
            side_effect=lambda player_id: self.mock_game_state.players[player_id].score
        )
        ...
        self.mock_game_state.players = {1: player1_mock, 2: player2_mock}
        ...
        self.mock_field_adapter.get_all_numbers = MagicMock(
            return_value=[
                [[None for _ in range(LENGTH_OF_SIDE)] for _ in range(LENGTH_OF_SIDE)] for _ in range(LENGTH_OF_SIDE)
            ]
        )
    ...
    def test_sys_04_two_player_game(self):
        ...
        self.mock_game_state.add_score.side_effect = lambda value: setattr(
            self.mock_game_state.players[self.mock_game_state.current_player_id],
            "score",
            self.mock_game_state.players[self.mock_game_state.current_player_id].score + value,
        )
    ...
    def test_sys_06_reset_game(self):
        ...
        self.mock_game_state.reset_game.assert_called_once_with(
            N=self.mock_game_state.N, is_ta_mode=True, level=self.mock_game_state.level
        )
